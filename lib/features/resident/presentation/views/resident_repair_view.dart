...
class _ResidentRepairViewState extends State<ResidentRepairView> {
  ...
  final _categories = [
    {
      'group': 'Bathroom',
      'items': ['Bathtub', 'Toilet', 'Water Heater']
    },
    {
      'group': 'Kitchen',
      'items': ['Refrigerator', 'Oven', 'Dishwasher']
    },
    {
      'group': 'Living Room',
      'items': ['Air Conditioner', 'TV/Display']
    },
    {
  ...
  final DateTime _transferDate =
      DateTime(2022, 10, 10); // Example Transfer/Handover Date
  ...
  bool _checkWarranty() {
    final expiryDate = DateTime(
        _transferDate.year + 5, _transferDate.month, _transferDate.day);
      ...
      if (_isValidTime(picked)) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content:
                  Text('กรุณาเลือกเวลาในช่วง 09:30-12:00 หรือ 13:00-16:00'),
              ...
              children: [
                Positioned.fill(
                    child: Opacity(
                        opacity: 0.015,
                        child: CustomPaint(
                            painter: _GridPainter(color: primaryGlow)))),
                Padding(
                  padding:
  ...
  Widget _buildHistoryPanel(Color gold) {
    return Column(
      ...
      children: [
        ...
        Text('Reviewing your past and active logs.',
            style: GoogleFonts.outfit(
                fontSize: 14, color: DashboardTheme.textPale)),
            ...
            builder: (context, repairs, _) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) => _HistoryCard(
                    item: repairs[index],
                    onTap: () =>
                        setState(() => _expandedTicket = repairs[index])),
              );
            },
          ),
        ),
  ...
  Widget _buildNoData() {
    return Center(
      ...
      Icon(Icons.query_builder_rounded,
          size: 40, color: DashboardTheme.textPale.withOpacity(0.1)),
      const SizedBox(height: 16),
      Text('Records Empty',
          style: GoogleFonts.outfit(color: DashboardTheme.textPale))
    ]));
  }
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              _StatusBadge(
                                  status: t.status, color: t.statusColor),
                          ...
                          Text(
                              'Technician Lead: ${t.technicianName ?? "Awaiting Assignment"}',
                              style: GoogleFonts.outfit(
                                  fontSize: 15,
                          ...
                          _ReviewRow(
                              label: 'Warranty Shield',
                              value: t.isWarranty ? 'ACTIVE' : 'EXPIRED',
                              isGreen: t.isWarranty),
                          ...
                          _OverlayBtn(
                              label: 'Close',
                              color: DashboardTheme.textMain.withOpacity(0.1),
                              isOutline: true,
                              onTap: () =>
                            ...
                            SizedBox(
                              ...
                              child: ListView.separated(
                                ...
                                itemBuilder: (context, idx) =>
                                    _PhotoPreview(path: t.imagePaths[idx]),
                              ),
                            ),
                          ],
                        ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      Text(
                          'This action will permanently remove the request from the registry history.',
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: hasVal
              ? activeColor
                  .withOpacity(DashboardTheme.isDarkMode.value ? 0.15 : 0.1)
              : DashboardTheme.surfaceSecondary,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
              color: hasVal
                  ? activeColor
                  : (DashboardTheme.isDarkMode.value
                      ? DashboardTheme.border.withOpacity(0.2)
              ...
              children: [
                Container(
                    ...
                    child: Icon(
                        ...
                        color:
                            hasPhotos ? activeColor : DashboardTheme.textPale)),
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      Text(hasPhotos ? 'ADD MORE PHOTOS' : 'ATTACH PHOTOS',
                          style: GoogleFonts.shareTechMono(
                              fontSize: 13,
                              color: hasPhotos
                                  ? activeColor
                                  : DashboardTheme.textPale.withOpacity(0.5),
                              fontWeight: FontWeight.bold)),
                      Text('${imagePaths.length} documents selected.',
                          style: GoogleFonts.outfit(
                              fontSize: 12, color: DashboardTheme.textPale)),
                    ])),
              ],
            ),
          ),
                ...
                children: [
                  ClipRRect(
                      ...
                      child: SizedBox(
                          ...
                          height: 100,
                          child: _PhotoPreview(
                              path: imagePaths[idx], isMini: true))),
                  Positioned(
                      top: 4,
                      right: 4,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: TextField(
          ...
          maxLines: maxLines,
          style:
              GoogleFonts.outfit(fontSize: 17, color: DashboardTheme.textMain),
          decoration: InputDecoration(
              hintText: hint,
              hintStyle: GoogleFonts.outfit(
                  fontSize: 16, color: DashboardTheme.textPale),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        ...
        hint: Text('Select Category',
            style: GoogleFonts.outfit(
                fontSize: 16, color: DashboardTheme.textPale)),
        items: items.expand((cat) {
          final g = cat['group'] as String;
          return (cat['items'] as List<String>).map((i) => DropdownMenuItem(
              value: '$g: $i',
              child: Text(i,
                  style: GoogleFonts.outfit(
                      fontSize: 16, color: DashboardTheme.textSecondary))));
        }).toList(),
                  ...
                  Text('URGENT DISPATCH',
                      style: GoogleFonts.shareTechMono(
                          ...
                          color: value
                              ...
                              : DashboardTheme.textPale.withOpacity(0.5),
                          letterSpacing: 1)),
                  Text('Priority maintenance enabled.',
                      style: GoogleFonts.outfit(
                          fontSize: 12,
...
class _Switch extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        padding: const EdgeInsets.all(4),
        decoration: BoxDecoration(
            color: value ? Colors.redAccent.withOpacity(0.2) : Colors.black45,
    ...
    return GestureDetector(
        ...
        child: Container(
            ...
            child:
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                _StatusBadge(status: item.status, color: item.statusColor),
                Text(item.date,
                    style: GoogleFonts.shareTechMono(
                        color: DashboardTheme.textPale, fontSize: 11))
              ]),
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final p = Paint()
      ...
      ..strokeWidth = 0.5;
    const s = 70.0;
    for (double i = 0; i < size.width; i += s) {
      canvas.drawLine(Offset(i, 0), Offset(i, size.height), p);
    }
    for (double i = 0; i < size.height; i += s) {
