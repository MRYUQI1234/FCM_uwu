    ...
    try {
      ...
      const result = requests.map((req: any) => {
        const tasks = db.prepare(`
          SELECT t.task_id, t.detail, t.prefer_date, t.status, t.task_report,
    ...
    try {
      ...
      db.transaction(() => {
        ...
        for (const task of tasks) {
          const taskId = uuidv4();
          // Validation: Time Slot (09:30 or 13:00)
          if (task.prefer_time !== '09:30:00' && task.prefer_time !== '13:00:00') {
            // If frontend sends '09:30' or '13:00', append ':00'
            if (task.prefer_time === '09:30') task.prefer_time = '09:30:00';
            else if (task.prefer_time === '13:00') task.prefer_time = '13:00:00';
            else throw new Error("ช่วงเวลาการซ่อมไม่ถูกต้อง อนุญาตเฉพาะ 09:30 หรือ 13:00 เท่านั้น");
          }
