...
class ResidentRepairView extends StatefulWidget {
  final bool isDark;
  final String? prefilledObjectName;

  const ResidentRepairView({
    super.key,
    required this.isDark,
    this.prefilledObjectName,
  });
...
class _ResidentRepairViewState extends State<ResidentRepairView> {
  ...
  // Object-to-category mapping for auto-fill from 3D model
  static const Map<String, String> _objectCategoryMap = {
    ...
    'bathroom basin': 'Bathroom: Bathtub',
    'bathroom mirror': 'Bathroom: Bathtub',
    'refrigerator': 'Kitchen: Refrigerator',
    'kitchen sink': 'Kitchen: Dishwasher',
    'kitchen stove': 'Kitchen: Oven',
    'air conditioner': 'Living Room: Air Conditioner',
    'ac remote': 'Living Room: Air Conditioner',
    'tv cabinet': 'Living Room: TV/Display',
    'smart panel': 'Living Room: TV/Display',
    'digital door lock': 'Infrastructure: Doors/Windows',
    'ceiling light': 'Infrastructure: Lighting',
    'spot light': 'Infrastructure: Lighting',
    'bollard': 'Infrastructure: Lighting',
    'light switch': 'Infrastructure: Lighting',
    'light fixture': 'Infrastructure: Lighting',
    'washing machine': 'Infrastructure: Plumbing',
    'dryer': 'Infrastructure: Plumbing',
    'wall': 'Infrastructure: Plumbing',
    'floor': 'Infrastructure: Plumbing',
  };

  @override
  void initState() {
    super.initState();
    _applyPrefill(widget.prefilledObjectName);
  }

  @override
  void didUpdateWidget(covariant ResidentRepairView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.prefilledObjectName != null &&
        widget.prefilledObjectName != oldWidget.prefilledObjectName) {
      _applyPrefill(widget.prefilledObjectName);
    }
  }

  void _applyPrefill(String? objectName) {
    if (objectName == null || objectName.isEmpty) return;
    _titleCtrl.text = objectName;
    final lower = objectName.toLowerCase();
    for (final entry in _objectCategoryMap.entries) {
