...
class _ResidentDashboardScreenState extends State<ResidentDashboardScreen>
  ...
  String? _prefilledObjectName;
  bool _repairArgHandled = false;
  ...
  Widget build(BuildContext context) {
    ...
    // Handle repairObject argument from 3D model view (one-shot)
    if (!_repairArgHandled && args is Map<String, dynamic> && args.containsKey('repairObject')) {
      _repairArgHandled = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          setState(() {
            _currentIndex = 1;
            _prefilledObjectName = args['repairObject'] as String?;
          });
        }
      });
    }
  ...
  Widget _buildView({required int index, required String displayUser, required bool isDark}) {
    switch (index) {
      ...
      case 1:
        final objectName = _prefilledObjectName;
        // Clear after use (one-shot)
        if (_prefilledObjectName != null) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) setState(() => _prefilledObjectName = null);
          });
        }
        return ResidentRepairView(key: ValueKey('repair_${objectName ?? "default"}'), isDark: isDark, prefilledObjectName: objectName);
      case 2:
