...
class _TechnicianViewScreenState extends State<TechnicianViewScreen>
    ...
    try {
      js.context.callMethod('eval', [
        ...
        (function() {
          ...
          window.toggleRoofTech = function(forceState) {
              if (forceState !== undefined) {
                  window._fcmTechRoofHidden = forceState;
              } else {
                  if (window._fcmTechRoofHidden === undefined) window._fcmTechRoofHidden = false;
                  window._fcmTechRoofHidden = !window._fcmTechRoofHidden;
              }
              const hide = window._fcmTechRoofHidden;
              ...
              const mv = document.getElementById('fcmHouseModelTechV2');
              if (mv) {
          ...
          window.fcmFocusTech = function(targetStr, zoomMult) {
              const mv = document.getElementById('fcmHouseModelTechV2');
              if (!mv) return;
              // model-viewer doesn't have a direct "jump to" JS API that works instantly for target
              // but we can set the attribute and it will interpolate
              mv.setAttribute('camera-target', targetStr);
              if (zoomMult) {
                 // Optionally adjust field of view or orbit for zoom
              }
          };
        })();
        """
      ...
      debugPrint("FCM Tech JS Init Error: $e");
    }
  }
  ...
  void _autoFocusTask(Map<String, dynamic> task) {
    if (!kIsWeb) return;
    
    final title = (task['title'] ?? '').toLowerCase();
    final report = (task['report'] ?? '').toLowerCase();
    
    // Auto-hide roof if it's an indoor issue
    bool isIndoor = title.contains('ไฟ') || report.contains('ไฟ') || 
                    title.contains('น้ำ') || report.contains('น้ำ') ||
                    title.contains('สวิตซ์') || title.contains('แอร์');
                    
    Timer(const Duration(milliseconds: 800), () {
        if (isIndoor) {
          js.context.callMethod('toggleRoofTech', [true]);
        } else {
          js.context.callMethod('toggleRoofTech', [false]);
        }
        
        String target = _getTaskFocusTarget(task);
        js.context.callMethod('fcmFocusTech', [target, 0.7]);
    });
  }

  String _getTaskFocusOrbit(Map<String, dynamic> task) {
    final title = (task['title'] ?? '').toLowerCase();
    final report = (task['report'] ?? '').toLowerCase();
    
    // Electrical / Light
    if (title.contains('ไฟ') || report.contains('ไฟ') || title.contains('สวิตซ์') || title.contains('switch')) {
      return '180deg 75deg 70%'; // Focused on internal wall switches/lights
    }
    // Water / Leak
    if (title.contains('น้ำ') || report.contains('น้ำ') || title.contains('รั่ว') || title.contains('leak')) {
      return '45deg 65deg 85%'; 
    }
    // AC / Air
    if (title.contains('แอร์') || report.contains('air') || title.contains('ร้อน')) {
      return '0deg 70deg 80%'; // Focused on AC units
    }
    // Structure / Wall
    if (title.contains('รอยร้าว') || title.contains('ผนัง') || title.contains('crack')) {
      return '90deg 80deg 90%';
    }
    
    // Default based on category
    return task['category'] == 'Maintenance' 
        ? '45deg 75deg 90%' 
        : (task['category'] == 'Security' ? '225deg 80deg 110%' : '45deg 65deg 100%');
  }

  String _getTaskFocusTarget(Map<String, dynamic> task) {
    final title = (task['title'] ?? '').toLowerCase();
    final report = (task['report'] ?? '').toLowerCase();

    if (title.contains('ไฟ') || report.contains('ไฟ') || title.contains('สวิตซ์')) {
      return '0m 1.2m 1m'; // Interior switch area
    }
    if (title.contains('น้ำ') || report.contains('น้ำ') || title.contains('รั่ว')) {
      return '1m 2.2m 0m'; // Ceiling area
    }
    if (title.contains('แอร์') || report.contains('air')) {
      return '-2m 2m 0m'; // AC unit area
    }

    return task['category'] == 'Maintenance' ? '0m 1.5m 0m' : 'auto 1.2m auto';
  }
            ...
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ▌ 3D VIEW — Functional interactive model
                  Text("REPAIR AREA", style: GoogleFonts.kanit(
                      fontSize: 11, color: _gold,
                      letterSpacing: 4, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 16),
                  Container(
                    height: 400, // Increased height for better interaction
                    width: double.infinity,
                    decoration: BoxDecoration(
                      color: _bgSidebar,
                      borderRadius: BorderRadius.circular(16),
                    ...
                    clipBehavior: Clip.antiAlias, // Important so model doesn't overflow corners
                    child: Stack(
                      children: [
                        ModelViewer(
                          key: const ValueKey('fcm_tech_model_v2_stable'),
                          id: 'fcmHouseModelTechV2',
                          ...
                          cameraControls: true,
                          disablePan: false,
                          cameraOrbit: _getTaskFocusOrbit(task),
                          cameraTarget: _getTaskFocusTarget(task),
                          minCameraOrbit: 'auto 45deg 0%',
                        ...
                        Positioned(
                          ...
                          child: MouseRegion(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                if (kIsWeb) js.context.callMethod('toggleRoofTech');
                              },
                              child: Container(
                                ...
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.layers_outlined, color: _gold, size: 18),
                                    const SizedBox(width: 8),
                                    Text("PEEL ROOF", style: GoogleFonts.kanit(color: _gold, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1)),
                                  ],
                                ),
                              ),
                            ),
                              ...
                              Positioned(
                                ...
                                child: Container(
                                  ...
                                  child: Text("📍 ${task['house']}",
                                      style: GoogleFonts.kanit(fontSize: 12, color: Colors.white70, fontWeight: FontWeight.w500)),
                                ),
                              ),
                            ],
                          ),
                          ...
                          child: IgnorePointer(
                            child: Center(
                              child: Text("Drag to rotate • Pinch to zoom",
                                  style: GoogleFonts.kanit(fontSize: 10, color: Colors.white30, letterSpacing: 1)),
                            ),
                          ),
                        ),
                      ],
